...
**Decision:** Use `_stack.frontend("./frontend")` for serving the static frontend build. `fas new` generates an empty `frontend/` directory as the drop target; users compile their SPA and place the output there. No HTML/JS starter content is generated by the scaffolder.
...
- `fastagentstack new` (with `include_frontend`) generates an empty `frontend/` directory as the drop target; users copy compiled SPA output there
