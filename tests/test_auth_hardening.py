...
async def _login(client: AsyncClient) -> tuple[str, str]:
    resp = await client.post(
        "/auth/token", json={"email": "alice@example.com", "password": "secret"}
    )
    data = resp.json()
...
async def test_b1_jti_written_to_denylist_on_logout(
...
async def test_b3_api_key_creation_returns_full_key_once(
...
async def test_b4_get_api_keys_has_no_key_field(
    client: AsyncClient, active_user: User
) -> None:
    bearer, _ = _bearer_for(active_user.id)
    await client.post(
        "/api-keys", json={"name": "k"}, headers={"Authorization": bearer}
    )
...
async def test_b5_authenticate_api_key_updates_last_used_at(
    client: AsyncClient, active_user: User
) -> None:
    bearer, _ = _bearer_for(active_user.id)
    create_resp = await client.post(
        "/api-keys", json={"name": "test"}, headers={"Authorization": bearer}
    )
...
async def test_b6_revoke_sets_revoked_at_not_delete(
    client: AsyncClient, active_user: User
) -> None:
    bearer, _ = _bearer_for(active_user.id)
    create_resp = await client.post(
        "/api-keys", json={"name": "rev-key"}, headers={"Authorization": bearer}
    )
    key_id = create_resp.json()["id"]
    rev_resp = await client.post(
        f"/api-keys/{key_id}/revoke", headers={"Authorization": bearer}
    )
    ...
    async for session in get_async_session():
        ...
        result = await session.execute(
            _select(ApiKey).where(ApiKey.id == uuid.UUID(key_id))
        )
...
def test_a1_i3_admin_lifespan_has_import_guard() -> None:
    src = (
        ...
        / "fast_agent_stack" / "core" / "admin" / "lifespan.py"
    )
...
def test_a2_i3_admin_views_has_import_guard() -> None:
    src = (
        pathlib.Path(__file__).parent.parent
        / "fast_agent_stack" / "core" / "admin" / "views.py"
    )
    tree = ast.parse(src.read_text())
    for node in ast.walk(tree):
        if isinstance(node, ast.Try):
...
def test_a3_i12_admin_no_internal_db_imports() -> None:
    admin_dir = (
        pathlib.Path(__file__).parent.parent / "fast_agent_stack" / "core" / "admin"
    )
            ...
            if isinstance(node, ast.ImportFrom) and node.module:
                for banned in forbidden:
                    if banned in node.module:
                        pytest.fail(
                            f"{py_file.name} imports from internal {node.module!r} (I12)"
                        )
...
async def test_n3_api_key_lookup_uses_key_hash_index(
    client: AsyncClient, active_user: User
) -> None:
    """Lookup queries by key_hash — proves indexed lookup path is exercised."""
    bearer, _ = _bearer_for(active_user.id)
    create_resp = await client.post(
        "/api-keys", json={"name": "n3"}, headers={"Authorization": bearer}
    )
    full_key = create_resp.json()["key"]
...
async def test_f2_expired_api_key_returns_none(
    client: AsyncClient, active_user: User
) -> None:
    bearer, _ = _bearer_for(active_user.id)
...
async def test_f3_revoked_api_key_returns_none(
    client: AsyncClient, active_user: User
) -> None:
    bearer, _ = _bearer_for(active_user.id)
    create_resp = await client.post(
        "/api-keys", json={"name": "revoked"}, headers={"Authorization": bearer}
    )
    key_id = create_resp.json()["id"]
    full_key = create_resp.json()["key"]
    # Revoke via route
...
async def test_f6_delete_api_key_removes_row(
    client: AsyncClient, active_user: User
) -> None:
    bearer, _ = _bearer_for(active_user.id)
    create_resp = await client.post(
        "/api-keys", json={"name": "del-key"}, headers={"Authorization": bearer}
    )
    key_id = create_resp.json()["id"]
